import "./CardSection.css";
import BookmarkDefault from "../../assets/bookmark.svg";
import BookmarkHover from "../../assets/bookmark-hover.svg";
import Dog from "../../assets/dog.png";
import Winter from "../../assets/winter.png";
import Mountain from "../../assets/mountain.png";
import { useState } from "react";
function CardSection() {
  const [hoveredCard, setHoveredCard] = useState(null);
  return (
    <div className="cards">
      <div className="cards__content">
        <div className="cards__list">
          <div className="cards__card">
            <div className="cards__header">
              <div
                onMouseEnter={() => setHoveredCard(0)}
                onMouseLeave={() => setHoveredCard(null)}
                className="cards__bookmark"
              >
                <img
                  src={hoveredCard === 0 ? BookmarkHover : BookmarkDefault}
                  alt="Book Mark Icon"
                />
              </div>
              <div className="cards__header-confirm-save">
                Sign in to save articles
              </div>
              <img src={Dog} alt="" className="cards__img" />
            </div>
            <div className="cards__texts-content">
              <div className="cards__texts-header">
                <div className="cards__date">November 4, 2020</div>
                <div className="cards__title">
                  Everyone Needs a Special 'Sit Spot' in Nature
                </div>
              </div>
              <div className="cards__description">
                Ever since I read Richard Louv's influential book, "Last Child
                in the Woods," the idea of having a special "sit spot" has stuck
                with me. This advice, which Louv attributes to nature educator
                Jon Young, is for both adults and children to find...
              </div>
              <div className="cards__footer-text">TREEHUGGER</div>
            </div>
          </div>
          <div className="cards__card">
            <div className="cards__header">
              <div
                onMouseEnter={() => setHoveredCard(1)}
                onMouseLeave={() => setHoveredCard(null)}
                className="cards__bookmark"
              >
                <img
                  src={hoveredCard === 1 ? BookmarkHover : BookmarkDefault}
                  alt="Book Mark Icon"
                />
              </div>
              <div className="cards__header-confirm-save">
                Sign in to save articles
              </div>
              <img src={Mountain} alt="" className="cards__img" />
            </div>
            <div className="cards__texts-content">
              <div className="cards__texts-header">
                <div className="cards__date">February 19, 2019</div>
                <div className="cards__title">Nature makes you better</div>
              </div>
              <div className="cards__description">
                We all know how good nature can make us feel. We have known it
                for millennia: the sound of the ocean, the scents of a forest,
                the way dappled sunlight dances through leaves. из местных чудес
                природы.
              </div>
              <div className="cards__footer-text">NATIONAL GEOGRAPHIC</div>
            </div>
          </div>
          <div className="cards__card">
            <div className="cards__header">
              <div
                onMouseEnter={() => setHoveredCard(2)}
                onMouseLeave={() => setHoveredCard(null)}
                className="cards__bookmark"
              >
                <img
                  src={hoveredCard === 2 ? BookmarkHover : BookmarkDefault}
                  alt="Book Mark Icon"
                />
              </div>
              <div className="cards__header-confirm-save">
                Sign in to save articles
              </div>
              <img src={Winter} alt="" className="cards__img" />
            </div>
            <div className="cards__texts-content">
              <div className="cards__texts-header">
                <div className="cards__date">October 19, 2020</div>
                <div className="cards__title">
                  Grand Teton Renews Historic Crest Trail
                </div>
              </div>
              <div className="cards__description">
                “The linking together of the Cascade and Death Canyon trails, at
                their heads, took place on October 1, 1933, and marked the first
                step in the realization of a plan whereby the hiker will be...
              </div>
              <div className="cards__footer-text">NATIONAL GEOGRAPHIC</div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
export default CardSection;
